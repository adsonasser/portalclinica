...
import { prontuarioApi, financialApi, usersApi, accessProfilesApi, appointmentTypesApi, contractTemplatesApi, whatsAppApi, contactTypesApi, settingsApi, leadsApi, patientsApi } from '../../services/api';
...
const MODULE_DETAIL: Record<string, { icon: string; label: string; desc: string; subKey: string }[]> = {
  ...
  contatos: [
    ...
    { icon: 'ti-file-import',  label: 'Importação',                desc: 'Importe pacientes em lote a partir de um arquivo CSV', subKey: 'contatos-importacao' },
  ],
  prontuario: [
...
const CSV_COL_MAP: Record<string, string> = {
  ...
  cpf:'cpf','data nascimento':'birthDate',data_nascimento:'birthDate',
  nascimento:'birthDate','dt nascimento':'birthDate',dt_nascimento:'birthDate',birthdate:'birthDate',
  sexo:'gender',genero:'gender',género:'gender',gender:'gender',
  cidade:'city',city:'city',estado:'state',uf:'state',state:'state',
  ...
  instagram:'instagram','como conheceu':'comoConheceu',como_conheceu:'comoConheceu',
  origem:'comoConheceu',responsavel:'responsible',responsável:'responsible',responsible:'responsible',
};
const CSV_FIELD_LABELS: Record<string,string> = {
  name:'Nome',email:'E-mail',phone:'Telefone',cpf:'CPF',birthDate:'Nascimento',
  gender:'Sexo',city:'Cidade',state:'UF',address:'Endereço',zipCode:'CEP',
  notes:'Observações',instagram:'Instagram',comoConheceu:'Como conheceu',responsible:'Responsável',
};
...
function ImportacaoView({ onBack, mc }: { onBack: () => void; mc: ModInfo }) {
  ...
  function processFile(file: File) {
    ...
    reader.onload = (e) => {
      ...
      h.forEach(col => { const k = CSV_COL_MAP[col.toLowerCase().trim()]; if (k) auto[col] = k; });
      setMapping(auto); setStep('preview');
    };
  ...
  function buildPatients() {
    return rawRows.map(row => {
      ...
      headers.forEach((col, i) => { if (mapping[col]) p[mapping[col]] = row[i] ?? ''; });
      return p;
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
          ...
          <div style={{ background:'#F4F4F5', borderRadius:10, padding:'14px 16px' }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:10 }}>
              {Object.values(CSV_FIELD_LABELS).map(l => <span key={l} style={{ fontSize:11, padding:'2px 8px', borderRadius:99, background:'#E4E4E7', color:'#52525B' }}>{l}</span>)}
            </div>
            <div style={{ fontSize:11, color:'#71717A', marginBottom:8 }}>
              Exemplo: <code style={{ background:'#E4E4E7', padding:'1px 5px', borderRadius:4, fontSize:11 }}>nome,email,telefone,cpf,data_nascimento,sexo,cidade,estado</code>
            </div>
            <button onClick={() => { const csv='nome,email,telefone,cpf,data_nascimento,sexo,cidade,estado\nMaria Silva,maria@email.com,(62) 99999-0000,000.000.000-00,1990-05-15,Feminino,Goiânia,GO\n'; const b=new Blob([csv],{type:'text/csv'}); const u=URL.createObjectURL(b); const a=document.createElement('a'); a.href=u; a.download='modelo_pacientes.csv'; a.click(); URL.revokeObjectURL(u); }}
              style={{ height:30, padding:'0 12px', border:'1px solid #D4D4D8', background:'#FFF', borderRadius:6, fontSize:12, fontWeight:500, color:'#18181B', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:5, fontFamily:'inherit' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          <div style={{ background:'#F4F4F5', borderRadius:10, padding:'14px 16px' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(190px,1fr))', gap:8 }}>
              {headers.map(col => (
                <div key={col}>
                  <div style={{ fontSize:11, color:'#71717A', fontWeight:500, marginBottom:3 }}>{col}</div>
                  <select value={mapping[col]??''} onChange={e => setMapping(m=>({...m,[col]:e.target.value}))} style={inp}>
                    <option value="">— ignorar —</option>
                    {Object.entries(CSV_FIELD_LABELS).map(([k,v]) => <option key={k} value={k}>{v}</option>)}
                  </select>
                </div>
              ))}
            </div>
          ...
          <div style={{ border:'1px solid #E4E4E7', borderRadius:10, overflow:'hidden' }}>
            <div style={{ padding:'10px 14px', borderBottom:'1px solid #E4E4E7', fontSize:12, fontWeight:600, color:'#09090B' }}>
              Prévia — {Math.min(5,patients.length)} de {patients.length} paciente(s)
            </div>
                ...
                <tbody>
                  {patients.slice(0,5).map((p,i) => (
                    <tr key={i} style={{ borderTop:'1px solid #F4F4F5' }}>
                      {uniqueFields.map(f => <td key={f} style={{ padding:'8px 12px', fontSize:12, color:'#18181B', whiteSpace:'nowrap', maxWidth:160, overflow:'hidden', textOverflow:'ellipsis' }}>{p[f]||'—'}</td>)}
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  const renderContent = () => {
    ...
    if (openSubItem) {
      switch (openSubItem) {
        ...
        case 'contatos-importacao':  return <ImportacaoView onBack={() => setOpenSubItem(null)} mc={mc} />;
